...
|--------|------|-------------|
| `POST` | `/publish` | Trigger a publish. Body: `{ "buildId": "...", "builderOrigin": "...", "buildMode": "ssg" \| "ssr" \| "cloudflare" \| "ssh" }` |
| `POST` | `/targets/ssh-setup` | Configure the SSH target for a domain (`buildMode: "ssh"`). Body: `{ "domain", "sshHost", "sshUser", "sshPath", "sshPort"?, "sshPrivateKey", "publicUrl"? }` |
| `POST` | `/unpublish` | Take a hostname down. Body: `{ "domain": "..." }` |
| `GET` | `/capabilities` | Publisher capabilities — `{ "cloudflare": bool, "ssh": true }` |
| `GET` | `/health` | Health check — returns `ok` |

### `buildMode: "ssh"` — deploy an SSG site to a remote server

Configure the target once, then publish with `buildMode: "ssh"`:
